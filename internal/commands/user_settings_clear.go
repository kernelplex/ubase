...
func UserSettingsClearCommand() ubcli.Command {
    const commandName = "user-settings-clear"
    ...
    run := func(args []string) error {
        ...
        resp, err := svc.UserSettingsRemove(context.Background(), ubmanage.UserSettingsRemoveCommand{
            Id:          id,
            SettingKeys: keys,
        }, agent)
        if err != nil {
        ...
        if resp.Status != ubstatus.Success {
            return fmt.Errorf("failed to clear user settings: %s", resp.Status)
        }
        fmt.Println("User settings cleared for provided keys.")
        return nil
    }

    return ubcli.Command{
        Name:    commandName,
        Help:    "Clear one or more user settings by key.",
        Run:     run,
        FlagSet: flagset,
    }
}
