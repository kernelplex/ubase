...
func RoleOverviewRoute(app *ubapp.UbaseApp, mgmt ubmanage.ManagementService, permissions []string) ubwww.Route {
    handler := func(w http.ResponseWriter, r *http.Request) {
        ...
        if more == "users" || more == "users/add" || more == "users/remove" {
            if r.Method == http.MethodPost {
                ...
                if uid <= 0 { http.Error(w, "Bad Request", http.StatusBadRequest); return }
                if more == "users/add" {
                    // add user to role
            ...
            if merr != nil { slog.Error("list users in role error", "error", merr, "role", id); members = []ubdata.User{} }
            memberSet := make(map[int64]bool, len(members))
            for _, u := range members { memberSet[u.UserID] = true }
            if r.Method == http.MethodPost {
                uidStr := r.FormValue("user_id")
                uid, _ := strconv.ParseInt(uidStr, 10, 64)
                user, gerr := adapter.GetUser(r.Context(), uid)
                if gerr != nil { slog.Error("get user error", "error", gerr, "user", uid); http.Error(w, "Not Found", http.StatusNotFound); return }
            ...
            if q == "" { users = members } else {
                const limit = 25
                var serr error
                users, serr = adapter.SearchUsers(r.Context(), q, limit, 0)
                if serr != nil { slog.Error("search users error", "error", serr); users = []ubdata.User{} }
            }
        ...
        if more == "permissions" || more == "permissions/add" || more == "permissions/remove" {
            ...
            writePermRow := func(w http.ResponseWriter, roleId int64, perm string, inRole bool) {
                ...
                rowClass := ""
                if inRole { rowClass = " class=\"row-in-role\"" }
                // Start row
            ...
            if r.Method == http.MethodPost {
                ...
                if perm == "" { http.Error(w, "Bad Request", http.StatusBadRequest); return }
                ...
                if aerr != nil { slog.Error("get role permissions error", "error", aerr, "role", id) }
                memberSet := make(map[string]bool, len(assigned))
                for _, p := range assigned { memberSet[p] = true }
            ...
            assigned, aerr := adapter.GetRolePermissions(r.Context(), id)
            if aerr != nil { slog.Error("get role permissions error", "error", aerr, "role", id); assigned = []string{} }
            memberSet := make(map[string]bool, len(assigned))
            for _, p := range assigned { memberSet[p] = true }
            ...
            if q == "" {
                ...
                for _, p := range permissions {
                    if strings.Contains(strings.ToLower(p), q) { filtered = append(filtered, p) }
                }
            }
            ...
            if len(filtered) == 0 {
                ...
                for _, p := range filtered { writePermRow(w, id, p, memberSet[p]) }
            }
...
func RoleCreateRoute(mgmt ubmanage.ManagementService) ubwww.Route {
    handler := func(w http.ResponseWriter, r *http.Request) {
        ...
        case http.MethodPost:
            if err := r.ParseForm(); err != nil { _ = views.RoleForm(isHTMX(r), false, nil, orgs, selectedOrg, "Invalid form submission", nil).Render(r.Context(), w); return }
...
func RoleEditRoute(mgmt ubmanage.ManagementService) ubwww.Route {
    handler := func(w http.ResponseWriter, r *http.Request) {
        ...
        case http.MethodGet:
            ...
            return
        case http.MethodPost:
            if err := r.ParseForm(); err != nil { _ = views.RoleForm(isHTMX(r), true, nil, orgs, 0, "Invalid form submission", nil).Render(r.Context(), w); return }
            name := strings.TrimSpace(r.FormValue("name"))
            sys := strings.TrimSpace(r.FormValue("system_name"))
