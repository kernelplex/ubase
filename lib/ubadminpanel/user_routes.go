...
func UsersListRoute(adapter ubdata.DataAdapter) ubwww.Route {
    ...
    return ubwww.Route{Path: "GET /admin/users", RequiresPermission: PermSystemAdmin, Func: handler}
}
...
func UserOverviewRoute(mgmt ubmanage.ManagementService) ubwww.Route {
    ...
    return ubwww.Route{Path: "GET /admin/users/{id}", RequiresPermission: PermSystemAdmin, Func: handler}
}

func UserRolesListRoute(mgmt ubmanage.ManagementService) ubwww.Route {
    handler := func(w http.ResponseWriter, r *http.Request) {
        idStr := r.PathValue("id")
        id, err := strconv.ParseInt(idStr, 10, 64)
        if err != nil || id <= 0 { http.NotFound(w, r); return }
        orgStr := r.URL.Query().Get("org")
        orgId, _ := strconv.ParseInt(orgStr, 10, 64)
        if orgId <= 0 { http.Error(w, "Bad Request", http.StatusBadRequest); return }
        rolesResp, err := mgmt.RoleList(r.Context(), orgId)
        if err != nil || rolesResp.Status != ubstatus.Success { http.Error(w, "Failed to load roles", http.StatusInternalServerError); return }
        hasResp, _ := mgmt.UserGetOrganizationRoles(r.Context(), id, orgId)
        memberSet := map[int64]bool{}
        if hasResp.Status == ubstatus.Success {
            for _, rr := range hasResp.Data { memberSet[rr.ID] = true }
        }
        _ = views.UserRolesTable(id, rolesResp.Data, memberSet, orgId).Render(r.Context(), w)
    }
    return ubwww.Route{Path: "GET /admin/users/{id}/roles", RequiresPermission: PermSystemAdmin, Func: handler}
}

func UserRolesAddRoute(mgmt ubmanage.ManagementService) ubwww.Route {
    handler := func(w http.ResponseWriter, r *http.Request) {
        idStr := r.PathValue("id")
        id, err := strconv.ParseInt(idStr, 10, 64)
        if err != nil || id <= 0 { http.NotFound(w, r); return }
        ...
        if orgId <= 0 || roleId <= 0 { http.Error(w, "Bad Request", http.StatusBadRequest); return }
        _, _ = mgmt.UserAddToRole(r.Context(), ubmanage.UserAddToRoleCommand{UserId: id, RoleId: roleId}, "web:ubadminpanel")
        hasResp, _ := mgmt.UserGetOrganizationRoles(r.Context(), id, orgId)
        memberSet := map[int64]bool{}
        if hasResp.Status == ubstatus.Success { for _, rr := range hasResp.Data { memberSet[rr.ID] = true } }
        rolesResp, _ := mgmt.RoleList(r.Context(), orgId)
        var role ubdata.RoleRow
        if rolesResp.Status == ubstatus.Success { for _, rr := range rolesResp.Data { if rr.ID == roleId { role = rr; break } } }
        _ = views.UserRoleRow(id, role, memberSet[roleId], orgId).Render(r.Context(), w)
    }
    return ubwww.Route{Path: "POST /admin/users/{id}/roles/add", RequiresPermission: PermSystemAdmin, Func: handler}
}

func UserRolesRemoveRoute(mgmt ubmanage.ManagementService) ubwww.Route {
    handler := func(w http.ResponseWriter, r *http.Request) {
        idStr := r.PathValue("id")
        id, err := strconv.ParseInt(idStr, 10, 64)
        if err != nil || id <= 0 { http.NotFound(w, r); return }
    ...
    return ubwww.Route{Path: "POST /admin/users/{id}/roles/remove", RequiresPermission: PermSystemAdmin, Func: handler}
}
...
func UserCreateRoute(mgmt ubmanage.ManagementService) ubwww.Route {
    ...
    return ubwww.Route{Path: "GET /admin/users/new", RequiresPermission: PermSystemAdmin, Func: handler}
}

func UserCreatePostRoute(mgmt ubmanage.ManagementService) ubwww.Route {
    handler := func(w http.ResponseWriter, r *http.Request) {
    ...
    return ubwww.Route{Path: "POST /admin/users/new", RequiresPermission: PermSystemAdmin, Func: handler}
}
...
func UserEditRoute(mgmt ubmanage.ManagementService) ubwww.Route {
    handler := func(w http.ResponseWriter, r *http.Request) {
        ...
        if r.Method == http.MethodGet {
            ...
            if err != nil || uresp.Status != ubstatus.Success { http.NotFound(w, r); return }
            st := uresp.Data.State
    ...
    return ubwww.Route{Path: "/admin/users/{id}/edit", RequiresPermission: PermSystemAdmin, Func: handler}
}
