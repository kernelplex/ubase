...
func TestUserCommandValidation(t *testing.T) {
	...
	if ok, _ := valid.Validate(); !ok {
		t.Fatal("expected valid create")
	}
	optional := UserCreateCommand{Email: "a2@b", Password: "Abcdef1!", Verified: true}
	if ok, _ := optional.Validate(); !ok {
		t.Fatal("expected valid create without names")
	}
	invalid := UserCreateCommand{}
	if ok, _ := invalid.Validate(); ok {
		t.Fatal("expected invalid create")
	}
	...
	if ok, _ := upd.Validate(); !ok {
		t.Fatal("expected valid update")
	}
	empty := ""
	updBlank := UserUpdateCommand{Id: 1, FirstName: &empty, LastName: &empty, DisplayName: &empty}
	if ok, _ := updBlank.Validate(); !ok {
		t.Fatal("expected valid update with blank names")
	}
	...
	if ok, _ := (UserGenerateVerificationTokenCommand{Id: 1}).Validate(); !ok {
		t.Fatal("expected valid token cmd")
	}
	if ok, _ := (UserGenerateVerificationTokenCommand{Id: 0}).Validate(); ok {
		t.Fatal("expected invalid token cmd")
	}
	...
	if ok, _ := gen.Validate(); !ok {
		t.Fatal("expected valid api key cmd")
	}
